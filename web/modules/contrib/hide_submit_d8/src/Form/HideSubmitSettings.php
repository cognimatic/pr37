...
class HideSubmitSettings extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['hide_submit_disable']['hide_submit_html_elements'] = [
      '#type' => 'textarea',
      '#title' => $this->t('Also hide HTML element with these CSS selectors after click.'),
      '#description' => $this->t('Enter a comma-separated list of CSS selectors (ie "a.button, ul li a.special-item").'),
      '#default_value' => $this->config->get('hide_submit_html_elements'),
    ];
    ...
    $form['hide_submit_forms']['hide_submit_form_ids'] = [
      '#type' => 'textarea',
      '#title' => $this->t('Form ID\'s'),
      '#description' => $this->t('Specify forms by using their id\'s .
      Enter one id per line .
      An example id is \'hide-submit-settings\'.'),
      '#default_value' => $this->config->get('hide_submit_form_ids'),
    ];

    $options = ['disabled' => 'Hide for following forms' , 'enabled' => 'Show for following forms'];
    $negate = $this->config->get('hide_submit_negate');
    $show_hide = empty($negate) ? NULL : $negate;
    $form['hide_submit_forms']['hide_submit_negate'] = [
      '#type' => 'radios',
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $config->set('hide_submit_hide_text', $form_state->getValue('hide_submit_hide_text'));
    ...
    $config->set('hide_submit_html_elements', $form_state->getValue('hide_submit_html_elements'));
    $config->set('hide_submit_form_ids', ($form_state->getValue('hide_submit_form_ids')));
    $config->set('hide_submit_negate', $form_state->getValue('hide_submit_negate'));
    $config->save();
