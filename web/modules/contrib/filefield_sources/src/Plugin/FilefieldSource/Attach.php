...
class Attach implements FilefieldSourceInterface {
  ...
  public static function process(array &$element, FormStateInterface $form_state, array &$complete_form) {
    ...
    // original list of options so they can be restored in the line above.
      ...
      $form_state->set(['filefield_sources', $field_name, 'attach_options'], $options);
    }

    $description = t('This method may be used to attach files that exceed the file size limit. Files may be attached from the %directory directory on the server, usually uploaded through FTP.', ['%directory' => realpath($path)]);

    // Error messages.
    if ($options === FALSE || empty($settings['path'])) {
      $attach_message = t('A file attach directory could not be located.');
      $attach_description = t('Please check your settings for the %field field.', ['%field' => $instance->getLabel()]);
    }
  ...
  public static function settings(WidgetInterface $plugin) {
    ...
    $return['source_attach'] = [
      ...
      '#element_validate' => [[get_called_class(), 'filePathValidate']],
      '#weight' => 3,
    ];
