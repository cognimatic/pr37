...
class BackupMigrateCommands extends DrushCommands {
  ...
  public function quickBackup(array $options = self::QUICK_BACKUP_OPTIONS) {
    ...
    if ($is_interactive) {
      ...
      $destiantion = $options['destination_id'];
      ...
      backup_migrate_perform_backup($source, $destiantion, $profile_config);
    }
  }
  ...
  public function runInteractiveQuickBackup() {
    ...
    $dentination_options = DrupalConfigHelper::getDestinationSelector($bam, 'Backup Destination')['#options'];
    unset($dentination_options['upload']);
    $dentination_options = array_keys($dentination_options);
    $dentination_choice = $this->io()->choice(dt("Choose the destination."), $dentination_options);

    backup_migrate_perform_backup($source_options[$source_choice], $dentination_options[$dentination_choice], $config);
  }
