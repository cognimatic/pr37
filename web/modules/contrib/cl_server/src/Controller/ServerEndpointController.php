...
class ServerEndpointController extends ControllerBase {
   ...
   *   The module handler.
   */
  public function __construct(KillSwitch $cache_kill_switch, ComponentDiscovery $component_discovery, ModuleHandlerInterface $module_handler, RendererInterface $renderer) {
  ...
  public static function create(ContainerInterface $container): self {
    ...
    return new static($cache_kill_switch, $component_discovery, $module_handler, $renderer);
  }
  ...
  private function generateTemplate(Component $component, string $variant, array &$context): string {
    ...
    return empty($block_names) && $this->supportsInject
      ? $this->generateInjectTemplate($component, $variant, $context)
      : $this->generateComponentTemplate($component, $variant, $block_names, $context);
  }
  ...
  private function generateComponentTemplate(
    ...
    foreach ($block_values as $block_name => $block_value) {
      ...
      $value = $this->renderer->render($block_build);
      $blocks_template .= "  {% block $block_name %}" . PHP_EOL
        . "    $value" . PHP_EOL
        . "  {% endblock %}" . PHP_EOL;
      // Now that we have the data captures in blocks let's remove the props.
      unset($context[$block_name]);
    }
    // Concatenate all the template.
    ...
    $template .= PHP_EOL;
    $template .= $blocks_template . PHP_EOL;
